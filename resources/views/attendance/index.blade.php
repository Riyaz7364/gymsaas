<x-layouts.app>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
        <div class="gh-stat-card">
            <div class="gh-stat-icon" style="background:#e0f7f1;"><i class="fas fa-user-check" style="color:#0abf8e;"></i>
            </div>
        ...
        <div class="gh-stat-card">
            <div class="gh-stat-icon" style="background:#fef3c7;"><i class="fas fa-door-open"
        ...
        <div class="gh-stat-card">
            <div class="gh-stat-icon" style="background:#ede9fe;"><i class="fas fa-calendar-check"
            ...
            <div class="gh-card">
                <div class="gh-card-header flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                    ...
                    <div class="flex flex-wrap gap-2 items-center">
                        <a href="{{ gym_route('gym.attendance.generate-qr') }}" class="gh-btn gh-btn-primary gh-btn-sm">
                            <i class="fas fa-qrcode"></i> Generate Daily QR
                        </a>
                        {{-- Date filter --}}
                        <form method="GET" class="flex flex-wrap gap-2 items-center">
                            <input type="date" name="date" value="{{ $date }}" class="gh-input"
                                style="width:auto; padding:6px 10px;">
                            <button type="submit" class="gh-btn gh-btn-outline gh-btn-sm">Filter</button>
                            @if ($date !== today()->toDateString())
                                <a href="{{ gym_route('gym.attendance.index') }}" class="gh-btn gh-btn-sm"
                                    style="background:#f3f4f6; color:#6b7280;">Today</a>
                            @endif
                        </form>
                    </div>
                </div>
                                    ...
                                    <tr>
                                        ...
                                        <td style="font-size:13px;">
                                            @if ($rec->duration_minutes !== null)
                                                @php
                                                    $h = intdiv($rec->duration_minutes, 60);
                                                    $m = $rec->duration_minutes % 60;
                                                @endphp
                                                {{ $h > 0 ? "{$h}h " : '' }}{{ $m }}m
                                            @elseif(!$rec->check_out)
                                                <span class="gh-badge gh-badge-success"
                                                    style="font-size:10px;">Inside</span>
                                            @else
                                                —
                                            @endif
                                        </td>
                                        <td>
                                            <span
                                                class="gh-badge {{ $rec->method === 'qr' ? 'gh-badge-info' : 'gh-badge-muted' }}"
                                        ...
                                        <td>
                                            @if (!$rec->check_out)
                                                <form method="POST"
                                                    action="{{ gym_route('gym.attendance.check-out', [$rec]) }}"
                                                    style="margin:0;">
                                                    @csrf @method('PATCH')
            ...
            <div class="gh-card">
                ...
                <div class="gh-card-body">
                    ...
                    <div class="gh-form-group" style="margin-bottom:12px;">
                        ...
                        <input type="text" x-model="query" @input.debounce.300ms="search" class="gh-input"
                        ...
                        <template x-for="m in results" :key="m.id">
                            <div @click="selectMember(m)"
                                ...
                                <div>
                                    ...
                                    <div style="font-size:11px; color:#9ca3af;" x-text="m.member_no + ' · ' + m.phone">
                                    </div>
                                </div>
                            </div>
                        </template>
                    ...
                    <div x-show="selected">
                        <div
                            style="display:flex; align-items:center; justify-content:space-between; padding:10px 12px; background:#f0fdf4; border-radius:8px; margin-bottom:16px; border:1px solid #86efac;">
                            <div style="display:flex; align-items:center; gap:8px;">
                        ...
                        <form method="POST" action="{{ gym_route('gym.attendance.check-in') }}">
                            @csrf
                    ...
                    async search() {
                        ...
                        try {
                            const resp = await fetch(
                                `{{ gym_route('gym.attendance.search-members') }}?q=${encodeURIComponent(this.query)}`, {
                                    headers: {
