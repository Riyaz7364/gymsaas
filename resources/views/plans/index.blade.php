<x-layouts.app>
    ...
    <div class="gh-card">
        <div class="gh-card-header">
            <h3 class="gh-card-title">All Plans <span
                    style="color:#9ca3af; font-weight:400; font-size:13px;">({{ $plans->count() }})</span></h3>
        ...
        @if ($plans->isEmpty())
            <div class="gh-card-body" style="text-align:center; padding:60px;">
                ...
                <p style="color:#9ca3af; font-size:14px; margin-bottom:16px;">Create your first membership plan to start
                    assigning members.</p>
                <a href="{{ gym_route('gym.plans.create') }}" class="gh-btn gh-btn-primary">+ Create Plan</a>
            </div>
        @else
            <div
                style="display:grid; grid-template-columns:repeat(auto-fill, minmax(280px, 1fr)); gap:16px; padding:20px;">
                @foreach ($plans as $plan)
                    <div
                        style="border:{{ $plan->is_active ? '2px solid #0abf8e' : '1px solid #e5e7eb' }}; border-radius:12px; padding:24px; background:#fff; position:relative;">
                        @if (!$plan->is_active)
                        ...
                        <div
                            style="font-size:12px; color:#9ca3af; margin-bottom:4px; text-transform:uppercase; letter-spacing:.5px;">
                        ...
                        <div style="font-size:13px; color:#6b7280; margin-bottom:16px;">{{ $plan->duration_days }} days
                        </div>
                        ...
                        <div
                            style="display:flex; align-items:center; justify-content:space-between; padding-top:12px; border-top:1px solid #f3f4f6;">
                            <div style="font-size:12px; color:#9ca3af;">
                                <strong style="color:#374151;">{{ $plan->member_plans_count }}</strong> assignments
                            </div>
                            <div style="display:flex; gap:6px;">
                                <a href="{{ gym_route('gym.plans.edit', [$gym, $plan]) }}"
                                ...
                                <button type="button"
                                    onclick="Livewire.dispatch('open-delete-dialog', {{ json_encode([
                                        'title' => 'Remove Plan',
                                        'message' => 'Are you sure you want to remove this plan? This action cannot be undone.',
                                        'itemName' => $plan->name,
                                        'confirmText' => 'Remove',
                                        'cancelText' => 'Cancel',
                                        'formAction' => gym_route('gym.plans.destroy', [$gym, $plan]),
                                    ]) }})"
