<x-layouts.app>
    <x-slot:title>Add Workout Category — {{ config('app.name') }}</x-slot:title>
    <x-slot:header>Add Workout Category</x-slot:header>
    <x-slot:topbarTitle>Add Workout Category</x-slot:topbarTitle>
    <x-slot:breadcrumb>Home / <a href="{{ route('workout-categories.index') }}">Workout Categories</a> / Add</x-slot:breadcrumb>
    ...
    <div class="gh-card" style="max-width:480px;">
        ...
        <div class="gh-card-body">
            ...
            @endif
            <form method="POST" action="{{ route('workout-categories.store') }}">
                @csrf
