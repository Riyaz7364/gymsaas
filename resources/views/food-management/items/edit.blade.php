<x-layouts.app>
    <x-slot:title>Edit Food Item — {{ config('app.name') }}</x-slot:title>
    <x-slot:header>Edit Food Item</x-slot:header>
    <x-slot:topbarTitle>Diet & Nutrition</x-slot:topbarTitle>
    <x-slot:breadcrumb>Home / <a href="{{ gym_route('gym.food-items.index') }}" style="color:var(--gh-primary);text-decoration:none;">Food & Drinks</a> / Edit</x-slot:breadcrumb>

    <div class="gh-card" style="max-width:600px;">
        <div class="gh-card-body">
            <form method="POST" action="{{ gym_route('gym.food-items.update', [$gym, $foodItem]) }}">
                @csrf @method('PUT')
                ...
                <div style="margin-bottom:20px;">
                    ...
                    <input type="text" name="name" class="gh-input" value="{{ old('name', $foodItem->name) }}" placeholder="e.g., Chicken Breast" required>
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:20px;">
                    <div>
                        ...
                        <input type="text" name="serving_size" class="gh-input" value="{{ old('serving_size', $foodItem->serving_size) }}" placeholder="e.g., 100">
                    ...
                    <div>
                        ...
                        <select name="serving_unit" class="gh-input" required>
                            <option value="g" {{ old('serving_unit', $foodItem->serving_unit) == 'g' ? 'selected' : '' }}>Grams (g)</option>
                            <option value="ml" {{ old('serving_unit', $foodItem->serving_unit) == 'ml' ? 'selected' : '' }}>Milliliters (ml)</option>
                            <option value="oz" {{ old('serving_unit', $foodItem->serving_unit) == 'oz' ? 'selected' : '' }}>Ounces (oz)</option>
                            <option value="cup" {{ old('serving_unit', $foodItem->serving_unit) == 'cup' ? 'selected' : '' }}>Cup</option>
                            <option value="tbsp" {{ old('serving_unit', $foodItem->serving_unit) == 'tbsp' ? 'selected' : '' }}>Tablespoon</option>
                            <option value="tsp" {{ old('serving_unit', $foodItem->serving_unit) == 'tsp' ? 'selected' : '' }}>Teaspoon</option>
                            <option value="piece" {{ old('serving_unit', $foodItem->serving_unit) == 'piece' ? 'selected' : '' }}>Piece</option>
                        </select>
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:20px;">
                    <div>
                        ...
                        <input type="number" name="calories" class="gh-input" value="{{ old('calories', $foodItem->calories) }}" placeholder="0" min="0">
                    ...
                    <div>
                        ...
                        <input type="number" name="protein_g" class="gh-input" value="{{ old('protein_g', $foodItem->protein_g) }}" placeholder="0.00" min="0" step="0.01">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:20px;">
                    <div>
                        ...
                        <input type="number" name="carbs_g" class="gh-input" value="{{ old('carbs_g', $foodItem->carbs_g) }}" placeholder="0.00" min="0" step="0.01">
                    ...
                    <div>
                        ...
                        <input type="number" name="fat_g" class="gh-input" value="{{ old('fat_g', $foodItem->fat_g) }}" placeholder="0.00" min="0" step="0.01">
                ...
                <div style="margin-bottom:20px;">
                    ...
                    <input type="number" name="fiber_g" class="gh-input" value="{{ old('fiber_g', $foodItem->fiber_g) }}" placeholder="0.00" min="0" step="0.01">
                    @error('fiber_g')<div style="color:#ef4444;font-size:13px;">{{ $message }}</div>@enderror
                </div>

                <div style="display:flex;gap:12px;">
                    <button type="submit" class="gh-btn gh-btn-primary">Update Food Item</button>
