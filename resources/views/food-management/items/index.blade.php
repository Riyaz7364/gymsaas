<x-layouts.app>
    ...
    <x-slot:breadcrumb>Home / <a href="{{ route('food-items.index') }}" style="color:var(--gh-primary);text-decoration:none;">Food & Drinks</a></x-slot:breadcrumb>
    ...
    <div style="margin-bottom:16px;display:flex;gap:8px;flex-wrap:wrap;">
        <a href="{{ route('food-items.index') }}" class="gh-btn {{ !request('category') ? 'gh-btn-primary' : 'gh-btn-outline' }}">All Foods</a>
        @foreach($categories as $category)
        <a href="{{ route('food-items.index', ['category' => $category->id]) }}" class="gh-btn {{ request('category') == $category->id ? 'gh-btn-primary' : 'gh-btn-outline' }}">{{ $category->name }}</a>
        @endforeach
        <a href="{{ route('food-categories.index') }}" class="gh-btn gh-btn-outline">Manage Categories</a>
    </div>

    <div class="gh-card">
        <div class="gh-card-header">
            <h3 class="gh-card-title">Food & Drinks Library</h3>
            <a href="{{ route('food-items.create') }}" class="gh-btn gh-btn-primary">+ Add Food Item</a>
        </div>
        ...
        <div class="gh-card-body" style="padding:0;">
            <table class="gh-table">
                ...
                <tr>
                    ...
                    <td>
                        <a href="{{ route('food-items.edit', $item) }}" class="gh-btn gh-btn-outline gh-btn-sm">Edit</a>
                        <form method="POST" action="{{ route('food-items.destroy', $item) }}" style="display:inline;" onsubmit="return confirm('Delete this food item?');">
                            @csrf @method('DELETE')
