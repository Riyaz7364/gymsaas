<x-layouts.app>
    <x-slot:title>Edit Food Category — {{ config('app.name') }}</x-slot:title>
    <x-slot:header>Edit Food Category</x-slot:header>
    <x-slot:topbarTitle>Diet & Nutrition</x-slot:topbarTitle>
    <x-slot:breadcrumb>Home / <a href="{{ route('food-categories.index') }}" style="color:var(--gh-primary);text-decoration:none;">Categories</a> / Edit</x-slot:breadcrumb>
        ...
        <div class="gh-card-body">
            <form method="POST" action="{{ route('food-categories.update', $foodCategory) }}">
                ...
                <div style="margin-bottom:20px;">
                    ...
                    <input type="text" name="name" class="gh-input" value="{{ old('name', $foodCategory->name) }}" placeholder="e.g., Meat, Drinks, Vegetables" required>
                ...
                <div style="margin-bottom:20px;">
                    ...
                    <textarea name="description" class="gh-input" placeholder="Optional description" rows="4" style="resize:vertical;">{{ old('description', $foodCategory->description) }}</textarea>
                ...
                <div style="margin-bottom:20px;">
                    ...
                    <input type="text" name="icon" class="gh-input" value="{{ old('icon', $foodCategory->icon) }}" placeholder="e.g., 🍗 (emoji)">
                    @error('icon')<div style="color:#ef4444;font-size:13px;">{{ $message }}</div>@enderror
                </div>

                <div style="display:flex;gap:12px;">
                    <button type="submit" class="gh-btn gh-btn-primary">Update Category</button>
