    ...
    <form method="POST" action="{{ route('members.update', $member) }}" enctype="multipart/form-data">
        ...
        <div style="display:grid; grid-template-columns: 1fr 340px; gap:20px; align-items:start;">
            ...
            <div style="display:flex; flex-direction:column; gap:20px;">
                    ...
                    <div class="gh-card-body">
                        <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px;">
                            <div class="gh-form-group" style="margin:0;">
                                ...
                                <input type="text" name="name" class="gh-input @error('name') border-red-400 @enderror"
                                       value="{{ old('name', $member->name) }}" required>
                                @error('name')<p class="gh-form-error">{{ $message }}</p>@enderror
                            </div>
                            <div class="gh-form-group" style="margin:0;">
                                <label class="gh-label">Phone <span style="color:red;">*</span></label>
                                <input type="tel" name="phone" class="gh-input @error('phone') border-red-400 @enderror"
                                       value="{{ old('phone', $member->phone) }}" required>
                            </div>
                            <div class="gh-form-group" style="margin:0;">
                                <label class="gh-label">Email</label>
                                <input type="email" name="email" class="gh-input" value="{{ old('email', $member->email) }}">
                            </div>
                            <div class="gh-form-group" style="margin:0;">
                                ...
                                <select name="gender" class="gh-input">
                                    ...
                                    <option value="{{ $v }}" {{ old('gender', $member->gender) === $v ? 'selected' : '' }}>{{ $l }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="gh-form-group" style="margin:0;">
                            ...
                            <div class="gh-form-group" style="margin:0;">
                                ...
                                <input type="text" name="occupation" class="gh-input" value="{{ old('occupation', $member->occupation) }}">
                            </div>
                            <div class="gh-form-group" style="margin:0;">
                ...
                {{-- Fitness --}}
                <div class="gh-card">
                    <div class="gh-card-header"><h3 class="gh-card-title">Fitness Information</h3></div>
                    <div class="gh-card-body">
                        <div class="gh-form-group" style="margin:0 0 16px;">
                            ...
                            <select name="goal" class="gh-input">
                                ...
                                <option value="{{ $v }}" {{ old('goal', $member->goal) === $v ? 'selected' : '' }}>{{ $l }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="gh-form-group" style="margin:0;">
            ...
            <div style="display:flex; flex-direction:column; gap:20px;">
                ...
                <div class="gh-card">
                    ...
                    <div class="gh-card-body" style="text-align:center;">
                        <div x-data="{ preview: '{{ $member->avatar ? asset('storage/'.$member->avatar) : 'https://ui-avatars.com/api/?name='.urlencode($member->name).'&color=fff&background=0abf8e&size=96' }}' }">
                            <div style="width:88px; height:88px; border-radius:50%; background:#f1f5f9; margin:0 auto 16px; overflow:hidden; border:3px solid var(--gh-border);">
                ...
                {{-- WhatsApp --}}
                <div class="gh-card">
                    <div class="gh-card-header"><h3 class="gh-card-title">WhatsApp AI</h3></div>
                    <div class="gh-card-body">
                        <label style="display:flex; align-items:flex-start; gap:10px; cursor:pointer;">
                            ...
                            <div>
                                <div style="font-size:14px; font-weight:500;">Opt-in AI Messages</div>
                                <div style="font-size:12px; color:#6b7280; margin-top:2px;">Send diet & workout on check-in</div>
                            </div>
                        </label>
                    </div>
                </div>
