<x-layouts.app>
    ...
    <div class="gh-card" style="max-width:600px;">
        <div class="gh-card-header"><h3 class="gh-card-title">Body Measurement Entry</h3></div>
        <div class="gh-card-body">
            ...
            @endif
            <form method="POST" action="{{ route('body-stats.store') }}">
                @csrf
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                    ...
                    <div>
                        ...
                        <input type="number" name="weight" class="gh-input" value="{{ old('weight') }}" step="0.1" min="0">
                    </div>
                    <div>
                        <label class="gh-label">Height (cm)</label>
                        <input type="number" name="height" class="gh-input" value="{{ old('height') }}" step="0.1" min="0">
                    </div>
                    <div>
                        <label class="gh-label">Body Fat %</label>
                        <input type="number" name="body_fat_pct" class="gh-input" value="{{ old('body_fat_pct') }}" step="0.1" min="0" max="100">
                    </div>
                    <div>
                        <label class="gh-label">Waist (cm)</label>
                        <input type="number" name="waist" class="gh-input" value="{{ old('waist') }}" step="0.1" min="0">
                    </div>
                    <div>
                        <label class="gh-label">Chest (cm)</label>
                        <input type="number" name="chest" class="gh-input" value="{{ old('chest') }}" step="0.1" min="0">
                    </div>
                    <div>
                        <label class="gh-label">Arms (cm)</label>
                        <input type="number" name="arms" class="gh-input" value="{{ old('arms') }}" step="0.1" min="0">
                    </div>
                    <div style="grid-column:1/-1;">
