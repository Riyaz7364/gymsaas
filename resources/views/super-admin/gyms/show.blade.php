<x-layouts.super-admin>
    ...
    <div style="display:grid;grid-template-columns:1fr 340px;gap:16px;align-items:start;">
            ...
            <div style="background:#fff;border-radius:14px;border:1px solid #e2e8f0;overflow:hidden;">
                ...
                @if($gym->subscriptions->isEmpty())
                    <div style="padding:24px 22px;">
                        <p style="font-size:14px;color:#64748b;margin:0;">This gym does not have a direct subscription record. It is being managed under the subscriber profile.</p>
                        @if($gym->owner)
                            <a href="{{ route('super-admin.subscriptions.show', $gym->owner) }}" style="display:inline-flex;align-items:center;gap:6px;margin-top:14px;padding:9px 12px;background:#eff6ff;color:#1d4ed8;border-radius:10px;text-decoration:none;font-size:13px;font-weight:700;">Open Subscriber Profile</a>
                        @endif
                    </div>
                @else
                        ...
                        <tbody>
                            @foreach($gym->subscriptions as $sub)
                                ...
                                <tr style="border-top:1px solid #f1f5f9;">
                                    ...
                                    <td style="padding:10px 16px;font-weight:700;color:#0f172a;">Rs {{ number_format($sub->amount) }}</td>
        ...
        <div style="display:flex;flex-direction:column;gap:16px;">
            <div style="background:#fff;border-radius:14px;border:1px solid #e2e8f0;overflow:hidden;">
                <div style="padding:18px 22px;border-bottom:1px solid #f1f5f9;">
                    <h3 style="font-size:15px;font-weight:700;color:#0f172a;margin:0;">Subscriber</h3>
                </div>
                    ...
                    @if($gym->owner)
                        ...
                        <div style="display:flex;flex-direction:column;gap:8px;">
                            <div style="display:flex;justify-content:space-between;">
                                ...
                                <span style="font-size:12px;font-weight:600;color:{{ ($gym->owner->status ?? 'active') === 'active' ? '#166534' : '#991b1b' }};">{{ ucfirst($gym->owner->status ?? 'active') }}</span>
                            </div>
                            <div style="display:flex;justify-content:space-between;">
                        ...
                        <a href="{{ route('super-admin.subscriptions.show', $gym->owner) }}" style="display:inline-flex;align-items:center;gap:6px;margin-top:14px;padding:9px 12px;background:#eff6ff;color:#1d4ed8;border-radius:10px;text-decoration:none;font-size:13px;font-weight:700;">Open Subscriber Profile</a>
                    @else
                        <p style="font-size:14px;color:#94a3b8;text-align:center;padding:12px 0;margin:0;">No subscriber assigned</p>
                    @endif
                </div>
            </div>
            ...
            @if($gym->activeSubscription)
                ...
                <div style="background:#0f172a;border-radius:14px;padding:20px 22px;">
                    ...
                    <div style="font-size:26px;font-weight:800;color:#0abf8e;margin-bottom:8px;">Rs {{ number_format($activeSub->amount) }}<span style="font-size:14px;color:rgba(255,255,255,.4);font-weight:400;">/{{ $activeSub->billing_cycle === 'monthly' ? 'mo' : 'yr' }}</span></div>
