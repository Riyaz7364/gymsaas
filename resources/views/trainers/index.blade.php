            ...
            <table style="width:100%; border-collapse:collapse;">
                <thead>
                    <tr style="background:#f8fafc; border-bottom:2px solid #e5e7eb;">
                        <th style="text-align:left; padding:11px 20px; font-size:12px; font-weight:700; color:#6b7280; text-transform:uppercase; letter-spacing:0.4px;">Trainer</th>
                        <th style="text-align:left; padding:11px 16px; font-size:12px; font-weight:700; color:#6b7280; text-transform:uppercase; letter-spacing:0.4px;">Specialization</th>
                        <th style="text-align:left; padding:11px 16px; font-size:12px; font-weight:700; color:#6b7280; text-transform:uppercase; letter-spacing:0.4px;">Phone</th>
                        <th style="text-align:center; padding:11px 16px; font-size:12px; font-weight:700; color:#6b7280; text-transform:uppercase; letter-spacing:0.4px;">Members</th>
                        <th style="text-align:center; padding:11px 16px; font-size:12px; font-weight:700; color:#6b7280; text-transform:uppercase; letter-spacing:0.4px;">Status</th>
                        <th style="text-align:right; padding:11px 20px; font-size:12px; font-weight:700; color:#6b7280; text-transform:uppercase; letter-spacing:0.4px;">Actions</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr style="border-bottom:1px solid #f3f4f6; transition:background 0.1s;"
                        ...
                        <td style="padding:14px 20px;">
                            <div style="display:flex; align-items:center; gap:12px;">
                                <img src="{{ $trainer->avatar ? asset('storage/'.$trainer->avatar) : 'https://ui-avatars.com/api/?name='.urlencode($trainer->name).'&color=fff&background=0abf8e&size=40&bold=true' }}"
                                     style="width:40px; height:40px; border-radius:50%; object-fit:cover; flex-shrink:0; border:2px solid #e5e7eb;" alt="">
                                <div>
                                    <div style="font-size:14px; font-weight:700; color:#111827;">{{ $trainer->name }}</div>
                                    @if($trainer->experience_years)
                                    <div style="font-size:12px; color:#9ca3af;">{{ $trainer->experience_years }}y exp</div>
                                    @endif
                                </div>
                            </div>
                        </td>
                        ...
                        <td style="padding:14px 16px; text-align:center;">
                            <span style="font-size:14px; font-weight:700; color:#0abf8e;">{{ $trainer->members_count }}</span>
                        </td>
                        {{-- Status --}}
                        <td style="padding:14px 16px; text-align:center;">
                            <span class="gh-badge {{ $trainer->status === 'active' ? 'gh-badge-success' : 'gh-badge-muted' }}" style="font-size:11px;">
