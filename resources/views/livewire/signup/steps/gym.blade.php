<div
    class="signup-card wide"
    x-data="{
        loading: false,
        gymSlug: '',
        baseUrl: @js(rtrim(config('app.url'), '/')),
        syncGymSlug(value) {
            const slug = value
                .trim()
                .toLowerCase()
            ...
            this.gymSlug = slug ? `${this.baseUrl}/gym/${slug}` : '';
        }
    }"
    x-init="syncGymSlug($wire.gym_name || '')"
>
    ...
    <form wire:submit.prevent="saveGym" @submit="loading = true">
        ...
        <div class="su-form-group">
            ...
            <div class="su-input-wrap">
                ...
                <input type="text" id="gym_name" wire:model.defer="gym_name" class="su-input"
                    placeholder="e.g. PowerFit Gym" @input="syncGymSlug($event.target.value)" autofocus required>
            </div>
            @error('gym_name')
                <p class="su-error">{{ $message }}</p>
            @enderror
        </div>

        {{-- Gym Slug --}}
        <div class="su-form-group" x-show="gymSlug.length > 0" x-cloak>
            <label class="su-label" for="gym_slug_preview">Gym admin URL</label>
            <div class="su-input-wrap">
                <span class="su-input-icon">
                    <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                        stroke-width="1.8">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M13.19 8.69a4.5 4.5 0 010 6.364l-1.06 1.06a4.5 4.5 0 01-6.364-6.364l1.06-1.06m5.304 6.364a4.5 4.5 0 010-6.364l1.06-1.06a4.5 4.5 0 116.364 6.364l-1.06 1.06" />
                    </svg>
                </span>
                <input type="text" id="gym_slug_preview" class="su-input" x-bind:value="gymSlug" disabled>
            </div>
            <p class="su-footer-text" style="text-align:left; margin-top:6px;">Preview only. This field is not
                submitted.</p>
        </div>

        <div class="su-form-group">
            <label class="su-label" for="address">Address <span
                    style="font-weight:400; color:#9ca3af;">(optional)</span></label>
            <div class="su-input-wrap">
                <span class="su-input-icon">
                    <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                        stroke-width="1.8">
                ...
                <input type="text" id="address" wire:model.defer="address" class="su-input"
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px;">
            <div class="su-form-group" style="margin-bottom:0;">
                ...
                <div class="su-input-wrap">
                    <input type="text" id="city" wire:model.defer="city" class="su-input no-icon"
            ...
            <div class="su-form-group" style="margin-bottom:0;">
                ...
                <div class="su-input-wrap">
                    <input type="text" id="state" wire:model.defer="state" class="su-input no-icon"
